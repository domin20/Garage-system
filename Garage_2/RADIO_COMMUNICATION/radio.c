...
void send_radio_frame(uint8_t *data_bytes, uint8_t size, uint8_t address, uint8_t command)
{
	...
	frame[1] = address;			// drugi bajt = rozmiar danych w ramce
	frame[2] = command;
	frame[3] = size;
	...
	frame[size+5] = (check_bytes & 0xFF);		// zaladuj bajty sprawdzajace do ramki
		...
		if(cnt == size + 6)			// jesli odebralismy juz wszystkie dane..
			{
				...
				if(sum == check_bytes)	// jesli jest rowna..
				{
					...
					{
						data_frame[i] = frame[i+4];	// zapisujemy dane do bufora
					}
